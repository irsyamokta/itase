    ...
    <div class="grid grid-cols-1 md:grid-cols-3 items-center justify-center gap-4">
        ...
        <div class="flex gap-2 md:col-span-2 justify-end">
            ...
            @if ($tim?->order?->payment_status == null || $tim?->order?->payment_status == 'Canceled')
                <form id="delete-team-form" action="{{ route('team.destroy', Auth::user()->id) }}" method="POST">
                    @csrf
                    @method('delete')
...
<div class="space-y-6">
            ...
            <div class="md:col-span-7 lg:col-span-9 space-y-4">
                ...
                <div class="flex flex-col items-start gap-2">
                    <a href="#" class="text-purple-600 hover:underline"
                        onclick="openPDF('{{ asset('storage/' . $participants[0]->ktm) }}')">
                        Preview
                    </a>
                </div>
            </div>
    ...
    <!-- Members Loop -->
    @foreach ($participants->skip(1) as $key => $participant)
        <!-- Skip leader (index 0) -->
        <div class="p-6 bg-white rounded-md shadow-md border-2 border-accent">
            ...
            <div class="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-10 gap-6">
                ...
                <div class="md:col-span-7 lg:col-span-9 space-y-4">
                    <input type="text" placeholder="Nama Lengkap" name="name[]"
                        ...
                        class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" />
                    <input type="email" placeholder="Email" name="email[]" value="{{ $participant->email ?? '' }}"
                        readonly
                        class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" />
                    <input type="text" placeholder="Universitas" name="university[]"
                    ...
                    <div class="flex flex-col items-start gap-2">
                        <a href="#" class="text-purple-600 hover:underline"
                            onclick="openPDF('{{ asset('storage/' . $participant->ktm) }}')">
                            Preview
                        </a>
                    </div>
                </div>
...
<script>
    document.getElementById('delete-team-button').addEventListener('click', function() {
        Swal.fire({
            ...
            text: "Data tim Anda akan dihapus!",
            icon: 'warning',
